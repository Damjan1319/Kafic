...
const AdminDashboard = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [ordersRes, tablesRes, waitersRes, statsRes, menuRes, inventoryRes] = await Promise.all([
                ...
                axios.get('/api/tables-positions'),
            ...
            console.log('Statistics details:', {
                overallStats: statsRes.data.overallStats,
                productStats: statsRes.data.productStats,
                waiterStats: statsRes.data.waiterStats
            });
    ...
    const addProduct = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('/api/menu-items', productData);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
            {/* Header */}
            <div
                className="bg-gradient-to-r from-slate-800/90 to-slate-900/90 backdrop-blur-sm border-b border-slate-700/50 sticky top-0 z-50">
            ...
            {/* Navigation Tabs */}
            <div
                className="bg-gradient-to-r from-slate-800/80 to-slate-900/80 backdrop-blur-sm border-b border-slate-700/50 sticky top-16 z-40">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        {statistics ? (
                            <>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                    <div
                                        className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-xl p-4 text-center border border-slate-700/50 shadow-xl">
                                        <div
                                            className="text-2xl sm:text-3xl font-bold text-cyan-400 mb-2">{statistics.overallStats?.totalOrders || 0}</div>
                                    ...
                                    <div
                                        className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-xl p-4 text-center border border-slate-700/50 shadow-xl">
                                        <div
                                            className="text-2xl sm:text-3xl font-bold text-emerald-400 mb-2">{Number(statistics.overallStats?.totalRevenue || 0).toFixed(0)} RSD
                                        </div>
                                    ...
                                    <div
                                        className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-xl p-4 text-center border border-slate-700/50 shadow-xl">
                                        <div
                                            className="text-2xl sm:text-3xl font-bold text-indigo-400 mb-2">{Number(statistics.overallStats?.averageOrderValue || 0).toFixed(0)} RSD
                                        </div>
                                    ...
                                    <div
                                        className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-xl p-4 text-center border border-slate-700/50 shadow-xl">
                                        <div
                                            className="text-2xl sm:text-3xl font-bold text-purple-400 mb-2">{statistics.overallStats?.totalItems || 0}</div>
                                ...
                                <div
                                    ...
                                    <div className="px-4 py-3 border-b border-slate-700/50">
                                        <h3 className="text-lg font-semibold text-slate-100">Najprodavaniji
                                            proizvodi</h3>
                                    ...
                                    <div className="p-4 space-y-3">
                                        {statistics.productStats.slice(0, 5).map((product, index) => (
                                            <div key={index}
                                                 className="flex items-center justify-between bg-gradient-to-r from-slate-700/50 to-slate-800/50 rounded-lg p-3 border border-slate-600/50">
                                                <div className="flex items-center space-x-3">
                                                ...
                                                <span
                                                    className="text-sm font-bold text-emerald-400">{Number(product.totalRevenue || 0).toFixed(0)} RSD</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div
                                    ...
                                    <div className="p-4 space-y-3">
                                        {statistics.waiterStats.map((waiter, index) => (
                                            <div key={waiter.id}
                                                 className="bg-gradient-to-r from-slate-700/50 to-slate-800/50 rounded-lg border border-slate-600/50 overflow-hidden">
                                                <div
                                                    className="p-3 cursor-pointer hover:bg-slate-600/30 transition-colors"
                                                    onClick={() => toggleWaiterExpansion(waiter.id)}>
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center space-x-3">
                                                            <span
                                                                className="text-sm font-semibold text-slate-100">{index + 1}.</span>
                                                            <div>
                                                                ...
                                                                <span
                                                                    className="text-xs opacity-75">orders handled</span>
                                                            </div>
                                                        </div>
                                                        <div className="flex items-center space-x-3">
                                                            <span
                                                                className="text-sm font-bold text-emerald-400">{Number(waiter.totalRevenue || 0).toFixed(0)} RSD</span>
                                                            <button
                                                ...
                                                {expandedWaiters.has(waiter.id) && (
                                                    <div className="px-3 pb-3 border-t border-slate-600/50">
                                                        <div className="grid grid-cols-2 gap-3 mt-3">
                                                            <div
                                                                className="bg-gradient-to-r from-blue-600/20 to-cyan-600/20 rounded-lg p-3 border border-blue-500/30">
                                                                <div className="text-center">
                                                                    <div className="text-sm font-bold text-blue-400">
                                                                        {waiter.ordersHandled > 0 ? Number((waiter.totalRevenue || 0) / waiter.ordersHandled).toFixed(0) : 0}
                                                                    </div>
                                                                    <div className="text-xs text-slate-400">RSD po
                                                                        porudžbini
                                                                    </div>
                                                                    <span
                                                                        className="text-xs opacity-75">RSD per order</span>
                                                                </div>
                                                            </div>
                                                            <div
                                                                className="bg-gradient-to-r from-emerald-600/20 to-teal-600/20 rounded-lg p-3 border border-emerald-500/30">
                                                                <div className="text-center">
                                                                    <div className="text-sm font-bold text-emerald-400">
                                                                        {waiter.ordersHandled > 0 ? Number((waiter.itemsSold || 0) / waiter.ordersHandled).toFixed(1) : 0}
                                                                    </div>
                                                                    <div className="text-xs text-slate-400">Stavki po
                                                                        porudžbini
                                                                    </div>
                                                                    <span
                                                                        className="text-xs opacity-75">Items per order</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                            ...
                            <div
                                className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-xl p-8 text-center border border-slate-700/50 shadow-xl">
                        ...
                        {orders.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {orders.map(order => (
                                    <div key={order.id}
                                         className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-xl border border-slate-700/50 shadow-xl overflow-hidden">
                                        <div className="p-4 cursor-pointer hover:bg-slate-700/30 transition-colors"
                                                ...
                                                <div className="space-y-2 mt-3">
                                                    {order.items.map((item, index) => (
                                                        <div key={index}
                                                            ...
                                                            <div className="flex items-center space-x-3">
                                                                <span
                                                                    className="text-sm font-semibold text-slate-100">{item.quantity}x</span>
                                                                <div>
                                                                    <h4 className="font-semibold text-slate-100 text-sm">{item.name}</h4>
                                                                    <p className="text-xs text-slate-400">{item.price} RSD
                                                                        komad</p>
                                                                    <span
                                                                        className="text-xs opacity-75">RSD per piece</span>
                                                                </div>
                                                            </div>
                    ...
                    <div className="space-y-6">
                        {user && user.role === 'admin' ? (
                            <>
                                ...
                                <div
                                    className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 text-xs sm:text-sm text-gray-400">
                            ...
                            <div className="space-y-4">
                                <div
                                    ...
                                    {tables.map(table => (
                                        <div key={table.id}
                                             className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-lg p-3 text-center border border-slate-700/50 shadow-lg">
                                            <div
                                                className="text-base font-semibold text-slate-100 mb-1">Sto {table.table_number}</div>
                ...
                {activeTab === 'waiters' && (
                    <div className="space-y-4">
                        <div className="grid gap-3">
                            {waiters.map(waiter => (
                                <div key={waiter.id}
                                     className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-sm rounded-xl p-4 border border-slate-700/50 shadow-xl">
                                    <div className="flex items-center justify-between">
                                        <div>
                    ...
                    <div className="space-y-6">
                        ...
                        {showAddProduct && (
                            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[9999] p-4">
                                <div
                                    className="bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-sm rounded-xl p-6 w-full max-w-lg border border-slate-700/50 shadow-2xl max-h-[90vh] overflow-y-auto">
                                    <div className="flex justify-between items-center mb-4">
                                        <div>
                                            <h3 className="text-xl font-semibold text-slate-100">Dodaj novi
                                                proizvod</h3>
                                    ...
                                    <form onSubmit={addProduct} className="space-y-4">
                                        <div>
                                            <label className="block text-sm font-medium text-slate-300 mb-2">Naziv
                                                proizvoda</label>
                                        ...
                                        <div>
                                            <label
                                                className="block text-sm font-medium text-slate-300 mb-2">Kategorija</label>
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-slate-300 mb-2">Cena
                                                (RSD)</label>
                                        ...
                                        <div>
                                            <label
                                                className="block text-sm font-medium text-slate-300 mb-2">Opis</label>
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-slate-300 mb-2">Početno
                                                stanje</label>
                            ...
                            {inventory.map(item => (
                                <div key={item.id}
                                    ...
                                    <div className="flex justify-between items-start mb-3">
                                        <h3 className="text-base font-semibold text-slate-100">{item.name}</h3>
                                        <span
                                            className={`text-sm font-bold ${item.stock > 10 ? 'text-emerald-400' : item.stock > 5 ? 'text-yellow-400' : 'text-red-400'}`}>
                      {item.stock} kom
