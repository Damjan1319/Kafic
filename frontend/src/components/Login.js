  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.info('Login successful', { username: data.user.username, role: data.user.role });
        
        // Login with user data (token is now in HttpOnly cookie)
      ...
      setError(error.message || 'Greška pri prijavi. Pokušajte ponovo.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mt-6">
          ...
          <p className="text-xs text-slate-500">
            © 2024 Sistem za Poručivanje u Kafiću
          </p>
        </div>
      </div>
    </div>
